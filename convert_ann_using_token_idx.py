...
def parse_parameters(parser=None):
    ...
    ## Required
    parser.add_argument("--input_ann", default="./ann", type=str)
    parser.add_argument("--input_tokens", default="./tokens", type=str)
    parser.add_argument("--output_ann", default="./tokens.ann", type=str)
...
def merge_consecutive_indices(indices: List[int]) -> List[int]:
    ...
    for i, v in enumerate(indices):
        if (i == 0) or (i == len(indices) - 1):
            # the latter index must follow 
            consecutive_indices.append(v)
        else:
            if i % 2 == 0:
                # the start of the latter (> 1) pairs
                if v > indices[i - 1] + 1:
                    # no need to merge!
                    consecutive_indices.append(v)
            else:
                # the end of the latter (> 1) pairs
                if v + 1 < indices[i + 1]:
                    # no need to merge!
                    consecutive_indices.append(v)
...
def load_token_boundaries(filepath):
    # filepath: the path of `tokens` file
    ...
    with open(filepath) as f:
        ...
        for line in f:
            if len(line.strip()) == 0 and pre_doc is not None:
                ...
                token_idx = 0
                continue
            # `token`, `document_name`, `start_index`, `end_index`
            sp = line.strip().split()
            assert len(sp) == 4
    ...
    with open(args.output_ann, "w") as out_f:
        with open(args.input_ann) as in_f:
            for line in in_f:
                ...
                # `document_name`, `sentence id`, `entity type`, `token indices`, `mention`
